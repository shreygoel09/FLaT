...
# -------# Jacobian Decoding #-------- #
def jacobian_decoding(x_k, classifier, optim_latent, mask, encoder_lm, eta, gamma, tau):
    x_k = x_k.clone().requires_grad_(True)
    x_flat = x_k.view(-1)

    # # Token relaxation with gumbel-softmax
    # gumbel_noise = -torch.log(-torch.log(torch.rand_like(x_k) + 1e-8) + 1e-8)
    # x_relaxed = (x_k + gumbel_noise) / tau    
    ...
    latent_loss = 0.5 * torch.sum(residual ** 2)

    # Vector-Jacobian product of relaxed logits wrt latent
    # (d_pi(x) / dx) * (pi(x)) - z')
    latent_grad = torch.autograd.grad(latent_loss, x_k, retain_graph=True)[0].clamp(min=-0.99, max=0.99)
    ...
    value_grad =  torch.autograd.grad(value_loss, x_k, retain_graph=True)[0]
    _print(f'value grad: {value_grad}')
    value_grad = (gamma * value_grad)#.clamp(min=0, max=0.99)
    _print(f'value grad: {value_grad}')

    # kl_div = F.kl_div(
    #     input=F.log_softmax(x_0, dim=-1),
    #     target=F.log_softmax(x_k, dim=-1),
    #     log_target=True,
    #     reduction='batchmean'
    # )
    # kl_grad = torch.autograd.grad(kl_div, x_k, retain_graph=True)[0]
    # _print(f'kl_grad: {kl_grad}')
